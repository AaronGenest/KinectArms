...
template<typename T>
void Serializer_<T>::Serialize(const std::vector<T>& elements, char data[], unsigned long& maxSize)
{
	...
	for(int i=0; i<elements.size(); i++)
	{
		...
		element.Serialize((char[])data[byteNum], bytesLeft);
		byteNum += bytesLeft;
	}
...
template<typename T>
void Serializer_<T>::Deserialize(const char data[], unsigned long& maxSize, std::vector<T>& elements)
{
	// Make sure T implements the ISerializable interface
	dynamic_cast<ISerializable&>(T());
	...
	for(int i=0; i<elements.size(); i++)
	{
		...
		unsigned long bytesLeft = maxSize - byteNum;
		element.Deserialize((char[])data[byteNum], bytesLeft);
		byteNum += bytesLeft;
	}
