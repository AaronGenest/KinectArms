...
void TestXorZlibEncoder(DepthImage& depthImage)
{
	/*
	...
	Codecs::EncodeXorZlib(depthImage, byteStream, length);
	//printf("Size: %u - %u\n", sizeof(depthImage), length);
	//printf("Perc: %f\n", length / (float)sizeof(depthImage));
	Codecs::DecodeXorZlib(byteStream, length, depthImage);
...
static void ExportDepthImageToCsv(const char* filename, const DepthImage& depthImage)
{
	...
	{
		fprintf(file, "%hu", depthImage[y][0]);

		for(int x=1; x<DEPTH_RES_X; x++)
		{
			fprintf(file, ",%hu", depthImage[y][x]);
		}
